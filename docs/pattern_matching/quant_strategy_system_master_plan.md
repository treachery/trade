...
## ⚡ 实现状态摘要（2026-06-30 更新）

| 优先级 | 需求编号 | 需求名称 | 实现状态 | 备注 |
|--------|----------|----------|----------|------|
| P0 | BE-000 | 项目模块骨架整理 | ✅ 已完成 | `pattern_matching/` 等模块已创建 |
| P0 | BE-001 | 业务对象与数据模型 | ✅ 已完成 | `models.py` 已实现 |
| P0 | BE-002 | Point-in-Time 规则 | ⚠️ 部分实现 | `time_gate.py` 存在但未完全集成 |
| P0 | BE-003 | A股交易约束规范 | ❌ 未实现 | 回测可能过于乐观 |
| P0 | QA-000 | 基础回归测试清单 | ❌ 未实现 | 无自动化测试 |
| P1 | BE-010 | 股票池管理 | ✅ 已完成 | `universe.py` 已实现 |
| P1 | BE-011 | 历史成分股快照 | ❌ 未实现 | 存在幸存者偏差风险 |
| P1 | BE-012 | 日K数据湖 | ⚠️ 部分实现 | 有缓存但非规范化数据湖 |
| P1 | FE-010 | 股票池配置组件 | ❌ 未实现 | 前端无配置界面 |
| P2 | BE-020 | 日频特征工厂 | ⚠️ 部分实现 | 特征计算分散在 `signals.py` |
| P2 | BE-021 | 独立信号事件引擎 | ✅ 已完成 | `signals.py` 已实现 |
| P2 | BE-022 | 信号扫描 API | ✅ 已完成 | `/api/pattern/signals` 已上线 |
| P2 | FE-020 | 信号事件列表与详情 | ❌ 未实现 | 无独立信号展示界面 |
| P3 | BE-030 | K线片段切片器 | ✅ 已完成 | `slicer.py` 已实现 |
| P3 | BE-031 | 片段标准化 | ✅ 已完成 | 集成在 `slicer.py` 中 |
| P3 | BE-032 | 逐级递进相似检索 | ✅ 已完成 | `retrieval.py` 已实现 |
| P3 | BE-033 | 相似检索 API | ✅ 已完成 | `/api/pattern/retrieval/query` 已上线 |
| P3 | FE-030 | 相似K线研究页 | ✅ 已完成 | `pattern_research.html` + `pattern.js` |
| P3 | FE-031 | 递进检索链路面板 | ❌ 未实现 | 缺少可解释性面板 |
| P3 | QA-030 | 相似检索验收用例 | ❌ 未实现 | 无自动化验收测试 |
| P4 | BE-040 | 候选策略库标准化 | ✅ 已完成 | `evaluator.py` 中 `STRATEGY_LIBRARY` |
| P4 | BE-041 | 相似样本策略评估 | ✅ 已完成 | `evaluator.py` 已实现 |
| P4 | BE-042 | 策略评估 API | ✅ 已完成 | `/api/pattern/analyze` 已上线 |
| P4 | FE-040 | 策略评估排名面板 | ⚠️ 部分实现 | 研究页中有折叠面板 |
| P5 | BE-050~052 | Walk-Forward 验证 | ❌ 未实现 | 核心验证机制缺失 |
| P6 | BE-060~062 | 持仓再评估与再平衡 | ❌ 未实现 | - |
| P7 | OPS-070~072 | 生产化自动化 | ⚠️ 部分实现 | 有 `_notify_daily_loop()` |
| P8 | BE-080~082 | 策略治理 | ❌ 未实现 | `governance/` 只有框架 |
...
2. **逐级递进相似K线检索**
   对触发强信号的标的，在历史片段库中执行 shape5 → shape10 → shape20 → shape30 四级递进检索，逐层收缩候选集（10000 → 1000 → 100 → 50）。
...
graph TB
    ...
    subgraph 第6层_检索["第6层：逐级递进相似检索引擎 ✅"]
        L1[shape5 近5日形态<br/>全历史→Top10000]
        L2[shape10 近10日形态<br/>Top10000→Top1000]
        L3[shape20 近20日形态<br/>Top1000→Top100]
        L4[shape30 近30日形态<br/>Top100→Top50]
    end

    subgraph 第5层_切片["第5层：K线片段切片与标准化 ✅"]
        SLICER[滑动窗口切片器<br/>250日窗口 / 步长1日]
    ...
    style 第10层_治理 fill:#fce4ec,stroke:#f44336,stroke-width:3px
    style 第9层_WF fill:#f3e5f5,stroke:#f44336,stroke-width:3px
    style 第8层_再平衡 fill:#e8f5e9,stroke:#f44336,stroke-width:3px
    style 第7层_策略评估 fill:#fff3e0,stroke:#4caf50,stroke-width:3px
    style 第6层_检索 fill:#e0f2f1,stroke:#4caf50,stroke-width:3px
    style 第5层_切片 fill:#ede7f6,stroke:#4caf50,stroke-width:3px
    style 第4层_信号 fill:#fbe9e7,stroke:#4caf50,stroke-width:3px
    style 第3层_特征 fill:#e8eaf6,stroke:#ff9800,stroke-width:2px
    style 第2层_数据湖 fill:#efebe9,stroke:#ff9800,stroke-width:2px
    style 第1层_接入 fill:#eceff1,stroke:#4caf50,stroke-width:3px
```
...
**流程说明**：
- **在线决策流程**（每日盘后）：第1层→第4层→第6层→第7层→第8层
- **离线验证流程**（定期执行）：第7层评估结果 → 第9层 Walk-Forward 验证 → 第10层治理决策
```
...
**职责**：对触发信号的标的，在历史片段库中执行四级递进检索，输出最相似的 Top50 片段（最终综合评分后取 Top10 展示）。

**设计依据**：将多周期特征拼接为单一高维向量的方案存在三方面缺陷：
1. 长窗口特征会稀释短期强信号的判别力；
2. 不同周期的特征权重缺乏可解释性；
3. 最终样本的选取逻辑难以追溯。
...
```text
先匹配极短期形态(shape5)，再匹配短期形态(shape10)，
接着匹配中期形态(shape20)，最后确认长期形态(shape30)一致
```
...
flowchart TD
    ...
    NEW_OPP --> SIM_RETRIEVAL[相似K检索<br/>shape5→shape10→shape20→shape30递进]
...
---
flowchart TD
    INPUT([输入：触发信号的标的<br/>+ 近30日K线序列 + 信号位向量]) --> STAGE1

    subgraph STAGE1["第1层：shape5 近5日形态"]
        F1[提取近5日K线形态向量<br/>6点对数收益率插值]
        F1 --> S1[全历史候选池矩阵化检索<br/>欧氏距离 @einsum]
        S1 --> R1[输出：Top10000<br/>近期形态最相似]
    end

    R1 --> STAGE2

    subgraph STAGE2["第2层：shape10 近10日形态"]
        F2[提取近10日K线形态向量<br/>12点对数收益率插值]
        F2 --> S2[Top10000内二次检索<br/>欧氏距离]
        S2 --> R2[输出：Top1000<br/>短期形态也相似]
    end

    R2 --> STAGE3

    subgraph STAGE3["第3层：shape20 近20日形态"]
        F3[提取近20日K线形态向量<br/>24点对数收益率插值]
        F3 --> S3[Top1000内三次检索<br/>欧氏距离]
        S3 --> R3[输出：Top100<br/>中期形态一致]
    end

    R3 --> STAGE4

    subgraph STAGE4["第4层：shape30 近30日形态"]
        F4[提取近30日K线形态向量<br/>30点对数收益率插值]
        F4 --> S4[Top100内四次检索<br/>欧氏距离]
        S4 --> R4[输出：Top50<br/>长期形态一致]
    end

    R4 --> FINAL_SCORE[综合评分<br/>形态相似度45% + 拟合度40% + 信号相似度15%]

    FINAL_SCORE --> OUTPUT([输出：Top10最相似历史片段<br/>每段含：标的/日期/相似度/<br/>形态距离/RMSE拟合度/信号Jaccard/<br/>后续N日走势])

    style INPUT fill:#e3f2fd
    style OUTPUT fill:#e8f5e9
    style FINAL_SCORE fill:#fff3e0
```

**每层特征详细定义**：

| 层级 | 形态采样点 | 特征维度 | 候选数 | 检索算法 |
|---|---|---|---|---|
| shape5 | 6点 | 近5日K线形态（对数收益率） | 10000 | 矩阵化欧氏距离 |
| shape10 | 12点 | 近10日K线形态（对数收益率） | 1000 | 矩阵化欧氏距离 |
| shape20 | 24点 | 近20日K线形态（对数收益率） | 100 | 矩阵化欧氏距离 |
| shape30 | 30点 | 近30日K线形态（对数收益率） | 50 | 矩阵化欧氏距离 |

**综合评分公式**：

```
最终得分 = 0.45 × 形态相似度 + 0.40 × 拟合度 + 0.15 × 信号相似度

其中：
- 形态相似度 = 1 / (1 + 加权形态距离)
  加权形态距离 = 0.40×shape5_dist + 0.30×shape10_dist + 0.20×shape20_dist + 0.10×shape30_dist
- 拟合度 = exp(-RMSE / 0.05)
  RMSE = sqrt(mean((查询shape20 - 候选shape20)²))（使用原始对数收益率，非标准化）
- 信号相似度 = Jaccard(查询信号位, 候选信号位)
  信号位 = 7维0/1向量（突破20d/60d/120d、布林带突破、量价突破、MA金叉、多头排列）
```

**实现细节**：

1. **形态向量提取**：使用对数收益率 `log(close[t-i] / close[t])` 消除量纲差异，线性插值到固定点数（6/12/24/30点）。
2. **矩阵化KNN**：候选池存储为 `(M, D)` float32 矩阵，查询时用 `@einsum` 一次性计算所有距离，比循环快10x+。
3. **特征标准化**：每层形态向量检索前执行 z-score 标准化（基于全历史分布），确保各维度可比；最终拟合度使用原始对数收益率（非标准化）计算 RMSE。
4. **信号位向量**：锚点处提取 T-2~T+2 窗口内的信号聚合（7维0/1向量），用于最终 Jaccard 相似度计算。
5. **可解释性**：每层记录输入/输出候选数及各样本相似得分，确保每条推荐可追溯（前端可展示递进检索链路）。
...
8. shape5/shape10/shape20/shape30 每层候选池及得分须完整记录。
9. 换仓须计入全部交易成本和滑点。
10. 空仓为合法动作。
...
- ✅ `pattern_matching/models.py`：统一业务对象与数据模型（BE-001 ✅）
- ✅ `pattern_matching/signals.py`：独立信号事件引擎（BE-021 ✅）
- ❌ 信号定义规范文档（代码中有但未系统化）
- ❌ 历史成分股数据深度清洗和校验（BE-011 未实现）
- ❌ 停牌/涨跌停/T+1/整数手约束规则的代码实现（BE-003 未实现）
- ❌ 未来数据泄漏检查清单（BE-002 未完全集成）

**待交付**：
- [ ] A股交易约束规则实现（T+1、整数手、涨跌停）→ 见 `TODO.md` P0
- [ ] 未来数据泄漏检查清单（自检脚本）→ 见 `TODO.md` P0
- [ ] 交易日历接入（A股官方交易日历）→ 见 `TODO.md` P0
...
| 模块 | 功能 | 实现文件 | 状态 |
|---|---|---|---|
| 数据层 | 股票池管理 | `pattern_matching/universe.py` | ✅ BE-010 |
| 信号引擎 | 10-20个规则信号实现 | `pattern_matching/signals.py` | ✅ BE-021 |
| 片段库 | 滑动窗口切片(250日/步长20日) | `pattern_matching/slicer.py` | ✅ BE-030 |
| 检索引擎 | shape5→shape10→shape20→shape30四级递进检索 | `pattern_matching/retrieval.py` | ✅ BE-032 |
| 策略评估 | 10个候选策略评估 | `pattern_matching/evaluator.py` | ✅ BE-041 |
| API | 信号扫描 + 相似检索 + 策略评估 | `app.py` `/api/pattern/*` | ✅ BE-022/033/042 |
| 前端 | 相似K线研究页 | `templates/pattern_research.html` + `static/pattern.js` | ✅ FE-030 |

**验收标准**：✅ 已满足
- ✅ 能对任意历史信号解释为什么推荐某策略（可追溯每一层候选池）
- ✅ 能列出每层相似检索的候选池和得分
- ✅ 能展示相似样本收益分布，而不只看平均收益
- ✅ 对于样本数不足30的情况，系统明确标注"置信度低，仅供参考"

**待完善**：
- ⚠️ 特征工厂未独立成模块（BE-020 部分实现，特征计算分散在 `signals.py`）
- ❌ 递进检索链路可解释面板（FE-031 未实现）→ 见 `TODO.md` P3
- ❌ 相似检索验收用例（QA-030 未实现）→ 见 `TODO.md` P3
...
**状态**：⚠️ 部分实现（2026-06-30）

**与现有代码衔接**：
当前 `app.py` 已有 `_notify_daily_loop()` 每日盘后自动扫描，可作为生产化的基础框架。

**已实现功能**：
- ✅ 信号扫描框架（`backtest/scanner.py`）
- ✅ Webhook/邮件推送（`app.py` 通知系统）
- ⚠️ 每日盘后自动化（有框架但未完善）

**待交付**（详见 `TODO.md` P7）：
- [ ] OPS-070 每日盘后任务编排（完善调度框架）
- [ ] OPS-071 盘后报告推送（针对相似K线策略）
- [ ] OPS-072 运行日志与健康检查
...
| 频率 | 日线 |
| 信号数量 | 10–20 个 |
| 观察窗口 | 5/10/20/30 个交易日四级递进 |
| 评估窗口 | 20 个交易日（后续走势观察期） |
| 递进候选数 | shape5 Top10000 → shape10 Top1000 → shape20 Top100 → shape30 Top50 |
| 片段窗口 | 250 个交易日 |
| 滑动步长 | 20 个交易日 |
...
- [x] ✅ **BE-000** 项目模块骨架整理（已完成）
- [x] ✅ **BE-001** 业务对象与数据模型（已完成）
- [ ] ❌ **BE-002** Point-in-Time 规则与泄漏检查规范（部分实现，未完全集成）
- [ ] ❌ **BE-003** A股交易约束规范实现（未实现，见 `TODO.md` P0）
- [ ] ❌ **QA-000** 基础回归测试清单（未实现，见 `TODO.md` P0）

### 优先级P1：数据湖 / 股票池

- [x] ✅ **BE-010** 证券主数据与股票池管理（已完成）
- [ ] ❌ **BE-011** 历史成分股快照存储（未实现，存在幸存者偏差风险）
- [ ] ⚠️ **BE-012** 日K数据湖与增量更新（部分实现，需升级为规范化数据湖）
- [ ] ❌ **FE-010** 股票池配置组件（未实现，前端无配置界面）

### 优先级P2：特征工厂 / 信号体系

- [ ] ⚠️ **BE-020** 日频特征工厂（部分实现，特征计算分散在 `signals.py`）
- [x] ✅ **BE-021** 独立信号事件引擎（已完成）
- [x] ✅ **BE-022** 信号扫描 API（已完成）
- [ ] ❌ **FE-020** 信号事件列表与详情（未实现）

### 优先级P3：K线片段库 / 相似检索

- [x] ✅ **BE-030** K线片段切片器（已完成）
- [x] ✅ **BE-031** 片段标准化与特征向量构建（已完成）
- [x] ✅ **BE-032** 逐级递进相似检索引擎（已完成）
- [x] ✅ **BE-033** 相似检索 API（已完成）
- [x] ✅ **FE-030** 相似K线研究页 MVP（已完成）
- [ ] ❌ **FE-031** 递进检索链路可解释面板（未实现）
- [ ] ❌ **QA-030** 相似检索验收用例（未实现）

### 优先级P4：策略评估

- [x] ✅ **BE-040** 候选策略库标准化（已完成）
- [x] ✅ **BE-041** 相似样本策略评估引擎（已完成）
- [x] ✅ **BE-042** 策略评估 API（已完成）
- [ ] ⚠️ **FE-040** 策略评估排名面板（部分实现，研究页中有折叠面板）

### 优先级P5：Walk-Forward 样本外验证（❌ 未实现）
