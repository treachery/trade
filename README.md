...
一个轻量、自用的 **A 股 / 港股 / 美股 / 指数** 策略回测平台：在线选标的与区间 → 自由组合多种入场/出场策略 → 在 K 线图上标注买卖点 → 给出总回报、夏普、卡玛、最大回撤等绩效（金额按市场自适应 ¥ / HK$ / US$）。支持 **一键寻优（3249 组合）**、**PE 估值仓位管理（含融资杠杆 + 按日计息）** 与 **PE 上升逐步卸杠杆**。

> 在线体验：http://49.233.175.122
（部署在腾讯云轻量服务器，Nginx + gunicorn + Docker）

技术栈：**akshare（免费 A 股/指数数据）+ yfinance（港股/美股数据）+ 自研逐日回测引擎 + Flask + ECharts**。
...
- **PE 仓位数据**支持：A股指数（乐咕长历史）、A股个股（百度近十年TTM）、港股指数/个股与美股个股（富途 OpenAPI 估值接口）、美股指数 `us.SPX`/`us.NDX`（WorldPERatio 月度长历史 PE）；其它美股指数暂无历史 PE 源，自动退化为满仓。
